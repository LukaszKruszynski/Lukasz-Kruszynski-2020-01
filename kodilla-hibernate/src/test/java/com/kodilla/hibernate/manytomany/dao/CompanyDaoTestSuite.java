...
public class CompanyDaoTestSuite {
    ...
    public void testFindByLastName() {
        //Given
        Employee adamExample = new Employee("Adam", "Example");

        //When
        employeeDao.save(adamExample);
        ArrayList<Employee> result = employeeDao.findByLastName("Example");
        int size = result.size();

        //Than
        Assert.assertEquals("Example", result.get(0).getLastName());
        Assert.assertNotEquals(0, size);

        //CleanUp
        employeeDao.delete(adamExample);
    }
    @Test
    public void testFindCompanyByFirstTreeCharacters() {
        //Given
        Company softwareMachine = new Company("Software Machine");
